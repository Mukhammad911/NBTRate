...
class Currencies extends Rate{
	...
	routeData(url,callback){
	  ...
	  switch (_url){
      case '/rate':this.getAllData(function (data) {
          callback(data);
      });break; 
	  
	  case '/rate/'+this.getRand(_url):this.getAllData(function (data) {
		  let i=0;
          while(data[i]){
      ...
      default:  callback({route:'Error route. Please contact: +992888881808. Turaev Munis'});;break;
      }
